    ...
    : BaseBusinessLogic(unitOfWork), IAppBusinessLogic
{
    ...
    public ICategoryService Categories => _categories ??= new CategoryService(unitOfWork, unitOfWork.Categories, mapper);
    
    private IIngredientService? _ingredients;
    public IIngredientService Ingredients =>
        _ingredients ??= new IngredientService(unitOfWork, unitOfWork.Ingredients, mapper);
    ...
    public IReviewService Reviews => _reviews ??= new ReviewService(unitOfWork, unitOfWork.Reviews, mapper);
    
    private IUnitService? _units;
    public IUnitService Units => _units ??= new UnitService(unitOfWork, unitOfWork.Units, mapper);
}